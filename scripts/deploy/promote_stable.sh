...
stable_service="${DEPLOY_STABLE_SERVICE:-axum-server}"
health_url="${DEPLOY_STABLE_HEALTHCHECK_URL:-}"
...
if [ -n "${health_url}" ]; then
  for attempt in $(seq 1 "${health_retries}"); do
    if curl -fsS --max-time 5 "${health_url}" > /dev/null; then
      echo "stable health check passed: ${health_url}"
      exit 0
    fi
  ...
  echo "stable health check failed after ${health_retries} attempts: ${health_url}"
  exit 1
fi
REMOTE
