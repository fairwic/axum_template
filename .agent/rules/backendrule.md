...
- **禁止**包含业务逻辑
- **禁止**直接调用 Repository
- 路径命名原则（使用 snake_case、路径包含函数名、使用{id}等）
...
- 操作当前用户自身资源时，**必须**从 Token (AuthUser) 获取 user_id
- **禁止**将当前用户 ID 放入 URL Path 参数（防止越权）
- 操作他人资源（如查看其他用户信息）可使用 Path 参数
- 路径命名规范：
  - `get_my_xxx` - 获取当前用户资源
  - `get_xxx/{id}` - 获取指定用户资源（公开信息）
  - `update_my_xxx` - 更新当前用户资源

```rust
// ❌ 错误：不应该从 Path 获取当前用户 ID
pub async fn get_my_stats(
    Path(id): Path<Ulid>,  // 危险：可能被篡改
) -> AppResult<...>

// ✅ 正确：从 Token 获取当前用户 ID
pub async fn get_my_stats(
    auth_user: AuthUser,  // 安全：来自验证后的 Token
) -> AppResult<...> {
    let user_id = auth_user.user_id;  // 使用 Token 中的 user_id
}
```
...
- 禁止任何语句使用 join,因为不排除我 join 的表后续会使用缓存
...
## utoipa/OpenAPI 自引用类型规则

### 自引用结构体

当 DTO 包含自引用字段（如 `children: Vec<Self>`）时，utoipa 生成 OpenAPI schema 会无限递归，导致栈溢出崩溃。

**解决方案**：在自引用字段上添加 `#[schema(no_recursion)]`
...
pub struct CommentResponse {
    pub id: String,
    pub children: Vec<CommentResponse>, // 自引用导致无限递归
}

// ✅ 正确：使用 no_recursion 打断递归
#[derive(Debug, Serialize, ToSchema)]
pub struct CommentResponse {
    pub id: String,
    #[schema(no_recursion)]  // 必须添加！
...
- **发现自引用字段时必须**添加 `#[schema(no_recursion)]`
- 典型场景：树形结构（评论回复、分类父子关系等）
- 症状：访问 `/api-docs/openapi.json` 时服务崩溃，日志显示 `stack overflow`
